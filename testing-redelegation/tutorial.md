...
1. Import the `createDelegation` function from `@metamask/smart-accounts-kit`
2. Create a delegation with `erc20TransferAmount` scope and identify the token by its address.
3. Sign the delegation using `aliceSmartAccount.signDelegation()`.

<Tabs>
<TabItem value="delegation.ts">

```typescript
// Import required functions and smart accounts
import { aliceSmartAccount, bobSmartAccount } from "./config.ts";
import { createDelegation } from '@metamask/smart-accounts-kit'
import { parseUnits } from 'viem'

// Create the delegation object with scope, participants, and environment
...
// Sign the delegation using Alice's smart account
...
```typescript
// Import required functions for smart account setup
...
1. Import the `createDelegation` function from `@metamask/smart-accounts-kit`.
2. Create a redelegation with the `parentDelegation` parameter.
3. Sign the redelegation using `bobSmartAccount.signDelegation()`.
...
// Create the redelegation with parent delegation from previous step
...
// Sign the redelegation using Bob's smart account
...
// Create Carol's account from private key
const carolAccount = privateKeyToAccount("0x...")

// Create Carol's smart account
