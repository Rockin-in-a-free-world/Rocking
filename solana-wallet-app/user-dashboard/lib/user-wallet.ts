 ...
 * 
 * Tether WDK SDK expects:
 * - `to` (not `recipient`) for the recipient address
 * - Returns an object with `signature` property (not just a string)
 */
export async function sendTransactionWithWDK(
  ...
  // Based on working example: account.sendTransaction({ to, value })
  const result = await account.sendTransaction({
    to: recipient, // Tether WDK expects 'to', not 'recipient'
    value: amountLamports,
    // commitment is optional, defaults to 'confirmed'
  });

  // Tether WDK returns an object with signature property
  // Example: { signature: string, fee?: bigint }
